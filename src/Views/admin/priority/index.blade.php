    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h2>{{ trans('ticketit::admin.priority-index-title') }}
                {!! link_to_route(
                                    Request::segment(1).'.'.$setting->grab('admin_route').'.priority.create',
        ...
        @if ($priorities->isEmpty())
            <h3 class="text-center">{{ trans('ticketit::admin.priority-index-no-priorities') }}
                {!! link_to_route(Request::segment(1).'.'.$setting->grab('admin_route').'.priority.create', trans('ticketit::admin.priority-index-create-new')) !!}
            </h3>
        @else
            <div id="message"></div>
            <table class="table table-hover">
                ...
                @foreach($priorities as $priority)
                    <tr>
                        ...
                        <td>
                            {!! link_to_route(
                                                    Request::segment(1).'.'.$setting->grab('admin_route').'.priority.edit', trans('ticketit::admin.btn-edit'), $priority->id,
                                                    ['class' => 'btn btn-info'] )
                                !!}

                                {!! link_to_route(
                                                    Request::segment(1).'.'.$setting->grab('admin_route').'.priority.destroy', trans('ticketit::admin.btn-delete'), $priority->id,
                                                    [
                            ...
                            {!! Form::open([
                                            'method' => 'DELETE',
                                            'route' => [
                                                        Request::segment(1).'.'.$setting->grab('admin_route').'.priority.destroy',
                                                        $priority->id
                                                        ],
