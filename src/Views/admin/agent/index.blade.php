    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h2>{{ trans('ticketit::admin.agent-index-title') }}
                {!! link_to_route(
                                    Request::segment(1).'.'.$setting->grab('admin_route').'.agent.create',
        ...
        @if ($agents->isEmpty())
            <h3 class="text-center">{{ trans('ticketit::admin.agent-index-no-agents') }}
                {!! link_to_route(Request::segment(1).'.'.$setting->grab('admin_route').'.agent.create', trans('ticketit::admin.agent-index-create-new')) !!}
            </h3>
        @else
            <div id="message"></div>
                    ...
                    <tr>
                        ...
                        <td>
                            {!! Form::open([
                                            'method' => 'PATCH',
                                            'route' => [
                                                        Request::segment(1).'.'.$setting->grab('admin_route').'.agent.update',
                                                        $agent->id
                                                        ],
                                            ]) !!}
                        ...
                        <td>
                            ...
                            'method' => 'DELETE',
                            'route' => [
                                        Request::segment(1).'.'.$setting->grab('admin_route').'.agent.destroy',
                                        $agent->id
                                        ],
                            'id' => "delete-$agent->id"
                            ]) !!}
