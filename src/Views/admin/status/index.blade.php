    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h2>{{ trans('ticketit::admin.status-index-title') }}
                {!! link_to_route(
                                    Request::segment(1).'.'.$setting->grab('admin_route').'.status.create',
        ...
        @if ($statuses->isEmpty())
            <h3 class="text-center">{{ trans('ticketit::admin.status-index-no-statuses') }}
                {!! link_to_route(Request::segment(1).'.'.$setting->grab('admin_route').'.status.create', trans('ticketit::admin.status-index-create-new')) !!}
            </h3>
        @else
            <div id="message"></div>
            <table class="table table-hover">
                ...
                @foreach($statuses as $status)
                    <tr>
                        ...
                        <td>
                            {!! link_to_route(
                                                    Request::segment(1).'.'.$setting->grab('admin_route').'.status.edit', trans('ticketit::admin.btn-edit'), $status->id,
                                                    ['class' => 'btn btn-info'] )
                                !!}

                                {!! link_to_route(
                                                    Request::segment(1).'.'.$setting->grab('admin_route').'.status.destroy', trans('ticketit::admin.btn-delete'), $status->id,
                                                    [
                            ...
                            {!! Form::open([
                                            'method' => 'DELETE',
                                            'route' => [
                                                        Request::segment(1).'.'.$setting->grab('admin_route').'.status.destroy',
                                                        $status->id
                                                        ],
                                            'id' => "delete-$status->id"
                                            ])
